...
app.get("/user/:id", (req, res) => {
  ...
  const query = "SELECT * FROM mydb2.user where id = ?";
...
app.post("/user", (req, res) => {
  ...
  dbcon.query(query, [name, profession], (err, result) => {
    if (err) {
      res.sendStatus(400).send(`<p>Error saving User with id : ${id}!</p>`);
    } else {
...
app.delete("/user/:id", (req, res) => {
  const id = req.params.id;
  const query = "DELETE FROM mydb2.user where id = ?";
  dbcon.query(query, [id], (err, result) => {
    if (err) {
...
app.put("/user/:id", (req, res) => {
  const id = req.params.id;
  const query = "UPDATE mydb2.user set name = ?, profession = ? where id = ?";
  const data = req.body;
  let name = data.name;
  let profession = data.profession;
